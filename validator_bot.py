...
def main():
    ...
    try:
        ...
        try:
            ...
            err_msg = f"Lỗi update_site_stats: {e}"
        ...
        try:
            ...
            err_msg = f"Lỗi process_priority_queue: {e}"
            logger.exception(err_msg)
            collected_errors.append(err_msg)
        ...
        try:
            ...
            err_msg = f"Lỗi process_pending_vouchers: {e}"
            logger.exception(err_msg)
            collected_errors.append(err_msg)
        ...
        try:
            ...
            err_msg = f"Lỗi recheck_live_vouchers: {e}"
            logger.exception(err_msg)
            collected_errors.append(err_msg)
        ...
        try:
            db.bot_health.update_one(
                {"_id": "validator_bot"},
                {"$set": {
                    "last_cycle_result": "error" if final_error_message else "success",
        ...
        logger.info(f"Hoàn thành chu kỳ kiểm duyệt. Kết quả: {'error' if final_error_message else 'success'}")
        import sys
